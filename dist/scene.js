...
export function startStation(parent,onReady) {
  ...
  class Station extends Phaser.Scene {
    ...
    setEvent(id){this.eventId=id||null;if(this.ready)this.applyEvent();}
    applyEvent(){
      if(!this.background||!this.eventOrb)return;this.tweens.killTweensOf(this.eventOrb);this.eventOrb.setScale(1).setAlpha(.8);this.background.clearTint();const look={LOW_GRAVITY:[0xa7d8ff,'◌'],METEOR:[0xffc77d,'✦'],AURORA:[0x9dffe0,'≋'],ECLIPSE:[0xc4a9ff,'◐']}[this.eventId];
      if(!look){this.eventOrb.setText('');return;}this.background.setTint(look[0]);this.eventOrb.setText(look[1]);if(!reduced)this.tweens.add({targets:this.eventOrb,scale:1.25,alpha:.35,duration:900,yoyo:true,repeat:-1,ease:'Sine.inOut'});
    }
    setRoster(players){
